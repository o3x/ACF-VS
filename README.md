...
*   **ハッシュベースの同一性判定**: SHA-256ハッシュを用いて判定するため、ファイル名が変更されても「同じファイル」として追跡します。同じファイルが前と違うパス（フォルダの変更・名前の変更）に移動された場合もそれを認識し、主に以下の種別に分類されて認識されます：
    * `[NEW]` (新規作成)
    * `[DELETED]` (削除)
    * `[UPDATED]` (内容の更新)
    * `[MOVED]` (ファイルパス・名前の変更)
...
*   **賢明な履歴管理と差分追跡**:
    ...
    * 前回から変更がない場合は、不要なファイルの上書きや履歴の追加を行わず安全にスキップします。
...
*   **エピソードルート (Root Dir)**:
    ...
    *   *※「参照...」ダイアログはWindowsの仕様上ファイル選択画面の形式ですが、フォルダを選択するためのものです。*
*   **カット番号**:
...
*   **Commit (Save) ボタン**:
    ...
    *   同時に、過去の状態がタイムスタンプ付きでバックアップされます。前回から変更がない場合は、保存処理をスキップします。
*   **History Log ボタン**:
...
*   **プログレスバー**:
    *   ファイル数の多いフォルダをスキャンする際の進捗状況をリアルタイムで視覚的に表示します。
*   **自動初期化ダイアログ (Auto Init)**:
    *   専用のInitボタンはありません。未管理のカットフォルダでScan等の操作を行った場合、自動的に「初期化（Init）して管理を開始しますか？」という確認ダイアログが表示され、ワンクリックで管理を開始できます。
