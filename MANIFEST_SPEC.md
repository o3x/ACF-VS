...
### 4-B. 連番グループ (Sequence Item)
`--seq` オプションによって自動グループ化された、2ファイル以上連続する連番画像のプロパティです。
...
"CELL/A_cell/A_cell_[0001-1000].tga": {
  "type": "sequence",
  "hash": "seq:4194304000:1709026000.0:1000",
  "size": 4194304000,
  "mtime": 1709026000.0,
  "is_archived": false,
  "count": 1000
...
*   **`hash`** (String): 連番代表ハッシュ (`"seq:<合計サイズ>:<最大mtime>:<枚数>"`)。グループ内全ファイルの合計サイズ・最新更新日時・枚数から構成されるため、グループ内のどのフレームが変更・削除されても値が変化し、変更を検知できます。
...
## 5. パーサーの実装例 (AfterEffects / ExtendScript_JSX)
